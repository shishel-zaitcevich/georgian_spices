...
type SpiceDetailsProps = {
  product: Product;
};

const SpiceDetails: React.FC<SpiceDetailsProps> = ({ product }) => {
  return (
    <div className={styles.details}>
      ...
      {product.spices && (
        <div className={styles.spices}>
          <h3>Included Spices</h3>
          <div className={styles.chips}>
            {product.spices.split(',').map((spice) => (
              <span key={spice.trim()} className={styles.chip}>
                {spice.trim()}
              </span>
            ))}
          </div>
...
export default memo(SpiceDetails);
